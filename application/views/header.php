<!DOCTYPE html>
<html>
	<head>
		<title>practice</title>

			<link rel="stylesheet" href="<?= base_url('assets/bootstrap/css/bootstrap.min.css') ?>">
 	 		<link rel="stylesheet" href="<?= base_url('assets/bootstrap/css/fontawesome.css') ?>">
  		<link rel="stylesheet" href="<?= base_url('assets/bootstrap/css/fontawesome.min.css') ?>">
  		<link rel="stylesheet" href="<?= base_url('assets/bootstrap/css/style.css') ?>">
  		<link rel="stylesheet" href="<?= base_url('assets/dist/sweetalert.css') ?>">
      <link rel="stylesheet" href="<?= base_url('assets/datatables/dataTables.bootstrap.min.css') ?>">
      <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
	</head>
<body>
	<div class="jumbotron jumbotron-danger text-center" style="background-color:#dc6900;">
      <h1 style="font-family:serif;"><i class="fa fa-drupal"></i> CODEIGNITER</h1> 
  <div class="dropdown" style="float:right; padding-right: 10px;">
    <button class="btn btn-warning " type="button" data-toggle="dropdown"><i class="fa fa-bars"></i> MENU
      <span class="caret"></span></button>
        <ul class="dropdown-menu dropdown-menu-left" style="padding-right:15px; ">
            <li><a class="icon-bar a" href="<?= base_url()?>"><i class="fa fa-home"></i> Home</a></li>
            <li><a class="icon-bar a" href="#" data-toggle="modal" data-target="#myReg"><i class="fa fa-edit"></i> Sign Up</a></li>
            <li><a class="icon-bar a" href="#" data-toggle="modal" data-target="#myLog"><i class= "fa fa-lock"></i> Login</a></li>
            <li><a class="icon-bar a" href="about"><i class="fa fa-info-circle"></i> about</a></li>
            <li><a class="icon-bar a" href="contact"><i class="fa fa-address-book"></i> Contact Us</a></li> 
        </ul>
  </div>
</div>



	
